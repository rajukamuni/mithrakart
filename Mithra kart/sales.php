...
<?php
	...
	if(isset($_GET['delete_id']))
	{
		...
		$stmt_select = $dbh->prepare('SELECT item_id FROM sales WHERE id =:id');
		$stmt_select->execute(array(':id'=>$_GET['delete_id']));
		$imgRow=$stmt_select->fetch(PDO::FETCH_ASSOC);
		// unlink("user_images/".$imgRow['userPic']);
		
		// it will delete an actual record from db
		$stmt_delete = $dbh->prepare('DELETE FROM sales WHERE id =:id');
			  ...
			  <tbody>
		        <?php
		          require 'database-config.php';
					$msg = '';
					$q = "SELECT sales.id, item_table.item_id, item_table.item_name, item_table.stock,item_table.sale_price, images.image, sales.customer, sales.customer_id, sales.quantity, sales.amount, address.email, address.full_name, address.delivery_mobile, address.pincode, address.house_no, address.colony, address.landmark, address.city FROM item_table INNER JOIN images INNER JOIN sales INNER JOIN address ON item_table.item_id=images.item_id AND item_table.item_id=sales.item_id AND sales.customer_id = address.customer_id WHERE images.image_number = '0'";
			    ...
			    <tr>
			      ...
			      <td><a href="?delete_id=<?php echo $row['id']; ?>" title="click when it delivered" onclick="return confirm('Are you sure? is it delivered ?')"><i class="fa fa-trash" aria-hidden="true"></i></i></a></td>
			    </tr>    
				<?php
						}
					}else{
		...
		<script>
			$("#exportBtn").click(function(){
				$("#salesTable").table2excel({
					// exclude: ".noExl",
					name: "sales",
					filename: "sales" //do not include extension
				});
			});
		</script>
</body>
