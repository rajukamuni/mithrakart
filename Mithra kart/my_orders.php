<?php 
  ...
  if(isset($_GET['delete_id']) && isset($_GET['user_id'])){
    $stmt_delete = $dbh->prepare('DELETE FROM user_orders WHERE customer_id =:id AND item_id = :itemId');
    $stmt_delete->bindParam(':id',$_GET['user_id']);
    $stmt_delete->bindParam(':itemId',$_GET['delete_id']);
    $stmt_delete->execute();
...
<html>
  ...
  <body>
    ...
    <div class="main-wrapper">
              ...
              $q = "SELECT item_table.item_id, item_table.item_name, item_table.stock, item_table.price, item_table.discount, item_table.sale_price, images.image, user_orders.quantity FROM item_table INNER JOIN images INNER JOIN user_orders ON item_table.item_id=images.item_id AND item_table.item_id=user_orders.item_id WHERE user_orders.customer_id=".$customer_id." AND images.image_number = '0'";
        ...
        <li class="col-3 col-md-1">
          <a href="?delete_id=<?php echo $row['item_id']; ?>&user_id=<?php echo $customer_id; ?>" title="click for delete" onclick="return confirm('sure to delete ?')"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
        </li>
      </ul>  
				<?php
						}
        ...
        <li class="col-12">
            <a href="#">No Item found in your orders. Happy shopping!</a>
        </li>
      </ul>
